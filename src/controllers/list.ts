...
export const addList = async (ctx: Context) => {
  ...
  try {
    await Board.updateOne({ _id: ctx.request.body.boardId }, { $push: { listsId: list._id } })
...
export const deleteList = async (ctx: Context) => {
  try {
    await Board.updateOne({ _id: ctx.request.body.boardId }, { $pull: { listsId: ctx.request.body.listId } })
...
export const updateList = async (ctx: Context) => {
  try {
    await List.updateOne({ _id: ctx.request.body.listId }, { title: ctx.request.body.title })
